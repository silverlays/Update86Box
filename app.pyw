...
class App(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self) -> None:
        ...
        if local.build == remote.last_build and local.roms_mtime >= remote.roms_mtime:
        ...
        if local.roms_mtime < remote.roms_mtime:
            ...
            self.updateRomsCheckBox.setText(
                self.updateRomsCheckBox.text() + " (update available)"
            )
    ...
    def launch86BoxManagerAndExit(self):
        self.writeSettings()
        if os.path.exists(self.commandLine[: self.commandLine.find(".exe") + 4]):
            subprocess.Popen(
                self.commandLine,
    ...
    def readSettings(self):
        try:
            with open("86BoxUpdater.json", "r") as fp:
                ...
                self.commandLine = str(fp_data["command_line"])
        except:
            pass
    ...
    def writeSettings(self):
        try:
            with open("86BoxUpdater.json", "w") as fp:
                ...
                json.dump(obj=json_data, fp=fp, indent=2)
        except:
            pass
